...
class Node(object) :
    def __init__(self,a , n = None, ):# Node object class definition with two attributes
        ...
        self.next_node  = n           #n is the reference to access the next node
    ...
    def set_next_node(self, n): # this method permits us to set the next node. Space = O(n), runtime = O(n)
        self.next.node = n      # it will allow us to step trough nodes
...
class CircularLinkedList(object): # Circular linked list class definition
    ...
    def insertion(self,a):            # this method is built to allow user to insert a link in the Circular linked List
        if self.get_size() == 0:       # in the case an if or else condition is true the program runtime = O(1),space(1)
            ...
            self.head = Node(a)
            self.head.set_next_node(self.root)
        else:
